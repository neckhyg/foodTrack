...
void StrToBytes(unsigned char * pszStr,  unsigned char * pszOutBytes, unsigned int nSize);
void StrXor(BYTE* SouStr, int iLen, BYTE* DesStr);
*/
//
// Constructor
//
...
//	sprintf(szPort, "COM%d", portnr);
	...
	if (m_hComm == INVALID_HANDLE_VALUE)
	{
		...
		delete [] szPort;
		delete [] szBaud;
        DWORD dwErrow = GetLastError(); 
		AfxMessageBox("Open Comm failed!");
		return FALSE;
	}
...
#endif
		unsigned char  pszOutBytes[128];
		StrToBytes((unsigned char *)port->m_szWriteBuffer,pszOutBytes,strlen((char*)port->m_szWriteBuffer));

		bResult = WriteFile(port->m_hComm,							// Handle to COMM Port
			pszOutBytes,								// Pointer to message buffer in calling finction
			strlen((char*)port->m_szWriteBuffer),	// Length of message to send
			&BytesSent,								// Where to store the number of bytes sent
			&port->m_ov);							// Overlapped structure
//void StrToBytes(unsigned char * pszStr,  unsigned char * pszOutBytes, unsigned int nSize)
